...
class EstudianteGrupoController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        //se valida que el estudiante que se va a crear no esté asociad a otro grupo
        $existingEstudianteGrupo =EstudianteGrupo::where('estudiante_id', $request->estudiante_id)->first();

        if ($existingEstudianteGrupo){
            return redirect()->back()->withInput()->withErrors(['estudiante_id' => 'Este estudiante ya pertenece a un grupo.']);
        }

        // Crear nueva relación EstudianteGrupo
        ...
        return redirect()->route('estudiantes_grupos.index')
        ->with('success', 'Estudiante grupo creado correctamente.');
    ...
    public function update(Request $request, $id)
    {
        ...
        // Actualizar solo el grupo al que pertenece el estudiante
        $estudianteGrupo->grupo_id = $request->grupo_id;
        $estudianteGrupo->save();

        return redirect()->route('estudiantes_grupos.index')
            ->with('success', 'Estudiante grupo actualizado correctamente.');
